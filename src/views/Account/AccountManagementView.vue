...
export default {
  ...
  async created() {
    await axios.getAllAccounts(this.offset, this.limit, this.firstname, this.lastname)
    .then(res => {
        res.forEach(item => {
            this.items.push({iban: item['iban'], type: item['type'], active: item['activated'], user: item['user_Id']})
        })
    })
  },
  methods: {
